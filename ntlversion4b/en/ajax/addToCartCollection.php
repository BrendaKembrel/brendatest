<?
	...
	$currency = $_POST["currency"]; //don't currently use this
	$defaultColor = $_POST["defaultColor"];
	
	//this is the statuses of the products we desire
	//a comma-sep'd list including a combo of retired,released,unreleased
	$productStatuses = $_POST["productStatuses"];
	...
	$priceString = $product->getPrice()->getFormattedPrice(
															array("class='productreg'", "class='productintermediateprice'", "class='productfinalprice'", "class='productyousave'"), 
															"",
															true, 
															false, 
															true);
	...
	$arrayOfActivationStatuses = array("released");//$productStatuses
	...
	$str="
		...
		<div style='float:left;margin:5px;width: 45%;text-align: left;'>
			<p class='producttitle'>".$product -> getProductName()."</p>"
			.$priceString.
			"<div class='productctawrapper'>
					
						<select onchange=\"getColorsForSize(true, '$productId', '$defaultColor', '$imagePath' )\" name='sizeSelectionColl' class='productselectyoursize'>";
